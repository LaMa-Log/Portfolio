  ...
  return (
    <ul role="list" className="space-y-2 sm:space-y-4">
      {array.map((item, index) => (
        <li
          key={index}
          className="border border-gray-700 rounded-2xl p-4 sm:p-6 shadow-md hover:shadow-lg transition-all duration-300"
        >
          {/* Titre + Statut */}
          ...
          {/* Séparateur */}
          <div className="w-full h-px bg-gray-600 mb-3 sm:mb-2" />

          {/* Description + Dates */}
          <div className="flex flex-col sm:flex-row items-start justify-between gap-3 text-sm sm:text-base text-gray-300">
            <p className="sm:w-3/4  text-xs md:text-sm lg:text-base leading-relaxed">{item.desc}</p>
            <p className="sm:w-1/4 font-medium text-xs sm:text-sm text-gray-400 text-end">
